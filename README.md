...
在 `.env.local` 設定僅供伺服器使用的 `OPENAI_API_KEY`、`OPENAI_VISION_MODEL`、`RATE_LIMIT_SECRET` 與 `ANALYSIS_TOKEN_SECRET`。兩個 secret 應各自使用至少 32 bytes 的隨機值。不要提交 `.env.local`，也不要把任何金鑰放在前端程式碼。
...
Playwright 會啟動本機伺服器，攔截分析、追問與遙測 API 並回傳固定假資料；它使用 `tests/fixtures/outfit-safe.png`（64×64、無真人的色塊服裝圖），不需金鑰，也不會把測試照片送到外部服務。測試涵蓋重拍、一次追問、回饋、錯誤重試、reload 清除狀態，以及 320／390／430px 版面。

## 安全與營運邊界

- 分析與追問 Route Handler 在讀取 body、解碼圖片或建立 AI client 前執行 same-origin、分 endpoint burst／sustained 與全域 concurrency guard。client signal 只會以 `RATE_LIMIT_SECRET` 做 HMAC，memory map 不保存 raw IP 或裝置 ID。
